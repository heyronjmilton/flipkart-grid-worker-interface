  ...
  waveform.register()
    ...
    const downloadXlsx = async () => {
      try {
        ...
        const response = await axios.get(`https://${host}/download-report`, {
...
async function sendFeedToServer(video) {
  ...
  function connectFeedWebSocket() {
      ...
      function connectObjectWebSocket() {
          objectwebsocket = new WebSocket(`wss://${host}/ws/fruits`);
  ...
  const onSensorOn = () => {
    const res = fetch(`https://${host}/set-in-sensor?value=1`);
  ...
  const onSensorOff = () => {
    const res = fetch(`https://${host}/set-in-sensor?value=0`);
    console.log("sensor off");
    toast.success("Sensor Off");
  }

  const onReset = () => {
    const res = fetch(`https://${host}/reset-detection`);
  ...
  const handleFormSubmit = async (e) => {
      ...
      const res = await fetch(`http://${host}/finish-task?batch_name=${batchName}&tasktype=fruit`, {
        method: 'POST',
        headers: {
