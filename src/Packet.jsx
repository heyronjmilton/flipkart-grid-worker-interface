...
const Packet = () => {
  ...
  const BorderLinearProgress = styled(LinearProgress)(({ theme }) => ({
    ...
    const downloadXlsx = async () => {
      try {
        ...
        const response = await axios.get(`http://${host}/download-report`, {
  ...
  const startCamera = async () => {
    ...
    async function sendFeedToServer(video) {
            ...
            feedwebsocket = new WebSocket(`ws://${host}/ws/camera_feed_expiry`);
      ...
      function connectObjectWebSocket() {
          objectwebsocket = new WebSocket(`ws://${host}/ws/packed_products_expiry`);
  ...
  const onSensorOn = () => {
    const res = fetch(`http://${host}/set-in-sensor?value=1`);
  ...
  const onSensorOff = () => {
    const res = fetch(`http://${host}/set-in-sensor?value=0`);
  ...
  const onReset = () => {
    const res = fetch(`http://${host}/reset-detection`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    const res = await fetch(`http://${host}/finish-task?batch_name=${batchName}&tasktype=packed`, {
      method: 'POST',
      headers: {
...
{isPopupVisible && (
      ...
      {loading && (
            ...
            <div className="bg-white bg-opacity-50 p-6 rounded-lg shadow-lg w-full max-w-md">
            <h2 className="text-xl font-bold mb-4">report is being generated</h2>
